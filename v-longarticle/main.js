...
fullText.addEventListener('input', function (e) {
  ...
  var textArr = [];
  while (index < len) {
    textArr.push(val.slice(index, index + 500));
    index += 500;
  }
  updateTextArea(textArr);
});
...
var textAreaList = [];
function updateTextArea(textArr) {
  var len = textArr.length - textAreaList.length;

  // prepare nodes
  if (len > 0) {
    for (var i = 0; i < len; i++) {
      var node = generateTextArea('');
      textAreaList.push(node);
      resultListNode.appendChild(node);
    }
  } else if (len < 0) {
    len = -len;
    for (var i = 0; i < len; i++) {
      resultListNode.removeChild(textAreaList[len - i - 1]);
      textAreaList.pop();
    }
  }
